...
def save_dev_note(page_title: str, note: str, frozen_at: str, page_state: dict) -> None:
    """Appends one Dev Mode note -- the page it was left on, the note text, and a
    ...
    on the page while the note was being written. JSONL (one JSON object per line),
    not CSV -- page_state is a nested dict of arbitrary widget values, not a flat row.
    Appending a line is also safe against a half-written file the way a full CSV
    rewrite wouldn't be."""
    ...
    entry = {"timestamp": frozen_at, "page": page_title, "note": note,
              "page_state": safe_state, "resolved": False}
