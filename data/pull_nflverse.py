...
def pull_schedules(years):
    ...
    validate(df, "schedules", key_cols=["game_id"], warn_null_cols=["home_score", "spread_line"])
    save(df, "schedules.csv")
    return df
...
def pull_pbp(years):
    ...
    validate(df, "play_by_play", key_cols=["game_id", "play_id"], warn_null_cols=["epa", "posteam"])
    save(df, "pbp.csv")
    return df
...
def pull_weekly_stats(years):
    ...
    validate(df, "weekly_stats", key_cols=["player_id", "season", "week"])
    save(df, "weekly_stats.csv")
    return df
...
def pull_ngs(years):
    ...
    for stat_type in ("passing", "rushing", "receiving"):
        ...
        validate(df, f"ngs_{stat_type}", key_cols=["player_gsis_id", "season", "week"])
        save(df, f"ngs_{stat_type}.csv")
        frames[stat_type] = df
    return frames
...
def pull_injuries(years):
    ...
    validate(df, "injuries", key_cols=["gsis_id", "season", "week"], warn_null_cols=["report_status"])
    save(df, "injuries.csv")
    return df


def pull_snap_counts(years):
    df = nfl.import_snap_counts(years)
    validate(df, "snap_counts", key_cols=["pfr_player_id", "season", "week"])
    save(df, "snap_counts.csv")
    return df
